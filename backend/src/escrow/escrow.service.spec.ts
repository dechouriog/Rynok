...
const mockRepo = () => ({ create: vi.fn(), save: vi.fn(), findOne: vi.fn(), update: vi.fn() });
...
describe('EscrowService', () => {
  ...
  beforeEach(async () => {
    const module = await Test.createTestingModule({
      providers: [
        EscrowService,
        { provide: getRepositoryToken(EscrowTransaction), useFactory: mockRepo },
        { provide: getRepositoryToken(Property), useFactory: mockRepo },
      ],
    }).compile();

    service = module.get(EscrowService);
    escrowRepo = module.get(getRepositoryToken(EscrowTransaction));
    propertyRepo = module.get(getRepositoryToken(Property));
  });

  it('registra un depósito y marca la propiedad como IN_ESCROW', async () => {
    const dto = {
      propertyId: 'prop-1', buyerWallet: '0xabc', sellerWallet: '0xdef',
      amountEth: '2', contractAddress: '0x123', depositTxHash: '0xhash',
    };
    escrowRepo.create.mockReturnValue(dto);
    escrowRepo.save.mockResolvedValue({ id: '1', ...dto });

    await service.recordDeposit(dto as any);

    expect(propertyRepo.update).toHaveBeenCalledWith('prop-1', { status: 'IN_ESCROW' });
  });

  it('rechaza confirmar si el solicitante no es el comprador', async () => {
    escrowRepo.findOne.mockResolvedValue({ buyerWallet: '0xabc' });
    await expect(service.confirmRelease('prop-1', '0xotra', '0xhash')).rejects.toThrow(ForbiddenException);
  });

  it('lanza NotFoundException si no hay depósito para esa propiedad', async () => {
    escrowRepo.findOne.mockResolvedValue(null);
    await expect(service.confirmRelease('prop-1', '0xabc', '0xhash')).rejects.toThrow(NotFoundException);
  });
});